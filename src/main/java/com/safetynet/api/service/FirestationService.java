...
public class FirestationService {
	...
	public FirestationCoveragePersonWithCounting getFirestationWithPersonInfo(String station) {
		...
		for (FirestationCoveragePerson person : firestationCoveragePersonWithCounting.getFireStationCoveragePerson()) {
			if (DefineMajority.isMinor(CalculAge.calculAgeWithBirthdate(
					medicalRecordDaoImpl.findBirthdateByName(person.getFirstName(), person.getLastName())))) {
				firestationCoveragePersonWithCounting
						.setCptChild(firestationCoveragePersonWithCounting.getCptChild() + 1);
			} else {
				firestationCoveragePersonWithCounting
						.setCptAdult(firestationCoveragePersonWithCounting.getCptAdult() + 1);
			}
		}
			...
			for (Firestation firestation : listFirestationAddress) {
				...
				for (Person person : personByAddress) {
					...
					personInfo.setMedications(
							medicalRecordDaoImpl.findMedicationsByName(person.getFirstName(), person.getLastName()));
					personInfo.setAllergies(
							medicalRecordDaoImpl.findAllergiesByName(person.getFirstName(), person.getLastName()));
					personInfo.setAge(CalculAge.calculAgeWithBirthdate(
							medicalRecordDaoImpl.findBirthdateByName(person.getFirstName(), person.getLastName())));
				}
